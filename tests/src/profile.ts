...
export default (orchestrator) => {
  ...
  orchestrator.registerScenario('test profile zomes', async (s, t) => {
    ...
    const [alice_xyzzy_happ, bob_xyzzy_happ] = await installAgents(conductor,  ["alice", "bobbo"])
    const [alice] = alice_xyzzy_happ.cells
    const [bobbo] = bob_xyzzy_happ.cells

    // Trigger init to run in both zomes
    await alice.call('xyzzy', 'list_channels', { category: "General" });
    await bobbo.call('xyzzy', 'list_channels', { category: "General" });
